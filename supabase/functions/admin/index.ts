// Supabase Edge Function: admin (PIN-validated via public.verify_pin — bcrypt + lockout). verify_jwt=false.
// Body: { actor_id, pin, op, payload }
// ops: add_rep | update_rep | delete_rep | set_rate | edit_entry | get_matrix | set_matrix | report | set_hours | my_pay
...
async function hashPin(pin) {
  const { data, error } = await admin.rpc('hash_pin', { p_pin: String(pin) });
  if (error) throw new Error('hash_pin: ' + error.message);
  return data;
}
...
Deno.serve(async (req) => {
  ...
  const actor = { id: actor_id, role: auth.role };
    ...
    switch (op) {
      case 'add_rep': {
        ...
        const { data, error } = await admin.from('app_users').insert({
          ...
          pin: await hashPin(p.pin), hourly_rate: p.hourly_rate ?? null, color: p.color ?? null, is_active: true,
      ...
      case 'update_rep': {
        ...
        if (p.pin) patch.pin = await hashPin(p.pin);
