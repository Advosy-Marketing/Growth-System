...
// Auth: FR_SUBDOMAIN / FR_AUTH_KEY / FR_AUTH_TOKEN (env or Vault).
// NOTE: routes must have the "API" group in "Who Can Schedule" (route
// settings in the FieldRoutes scheduling UI) or inserts are rejected.
// =====================================================================
...
// POST form-encoded; auth params LAST (truncated request can never half-execute).
...
export const frProvider: AvailabilityProvider = {
  async getAvailability(cfg, startMs, endMs, timezone) {
    ...
    // Keep only genuinely open spots (verified live: open="1" on free spots).
    const open = spots.filter((s) => {
      const isOpen = s?.open === undefined || String(s.open) === "1";
      const taken = pickId(s, "currentAppointment", "currentAppointmentID");
      const reserved = String(s?.reserved ?? "0") === "1";
      const blocked = String(s?.blockReason ?? "").trim() !== "";
      const api = s?.apiCanSchedule === undefined || String(s.apiCanSchedule) === "1";
      return isOpen && !taken && !reserved && !blocked && api;
    });
  ...
  async createBooking(cfg, input) {
    ...
    // When booking into a spot, do NOT send duration — spots hold ~24 min and
    // FieldRoutes rejects anything over capacity. Service type default applies.
    if (input.slot.ref) {
...
export interface SaleResult {
  ...
  contractLink?: string;   // e-sign portal URL returned by contract/create (reps can text it)
...
export async function frCreateSale(cfg: ServiceConfig, sale: SaleInput): Promise<SaleResult> {
  ...
  if (sale.emailContract !== false) {
    try {
      const cRes = await frFetch("contract/create", { subscriptionID, emailCustomer: 1 });
      contractSent = true;
      // contract/create returns the e-sign portal URL in `result` (verified live).
      const link = cRes?.result;
      if (typeof link === "string" && link.startsWith("http")) contractLink = link;
    } catch (e) {
      contractError = String((e as Error)?.message ?? e);
    }
  }

  return { customerID, subscriptionID, contractSent, contractLink, contractError };
}
