...
// verify_jwt = false; the actor's PIN + role are checked server-side (service role).
...
//     - list:           any active user. { date_from?, date_to?, limit? }
//     - update_session: admin/manager.   { session_id, patch }
//     - update_item:    admin/manager.   { item_id, patch }
//     - cancel:         admin/manager.   { session_id, reason? }
...
// Per-line token wins over the shared one, from either store (env or Vault),
// so cancel/reschedule hits the same GHL sub-account the appointment was booked in.
//   env GHL_TOKEN_<SERVICE> -> Vault GHL_TOKEN_<SERVICE> -> env GHL_TOKEN -> Vault GHL_TOKEN
  ...
  try {
    switch (op) {
      ...
      case "update_item": {
        ...
        // Best-effort sync to GHL for scheduled (roofing/restoration) items.
