...
async function secret(name:string){ const e=Deno.env.get(name); if(e) return e; try{ const { data } = await sb.rpc('get_secret',{p_name:name}); if(data) return String(data); }catch(_){} return ''; }

// AZ + NV bounding rectangle (lat/lng) for biasing/locating.
const RECT = { low:{ latitude:31.2, longitude:-120.3 }, high:{ latitude:42.2, longitude:-108.8 } };
const AZNV_RE = /,\s*(AZ|Arizona|NV|Nevada)\b/i;

// ---- Google Places (New) ----
async function autoNew(key:string, input:string){
  ...
  for(const s of (b.suggestions||[])){
    ...
    const desc = (p.text&&p.text.text) || '';
    if(desc && AZNV_RE.test(desc)) out.push({ placeId:p.placeId, description:desc });
  }
  return { ok:true, predictions: out.slice(0,8) };
}
async function detailsNew(key:string, placeId:string){
...
async function autoLegacy(key:string, input:string){
...
async function detailsLegacy(key:string, placeId:string){
  ...
  const res = b.result||{}; const comp:Record<string,string> = {};
...
function assemble(comp:Record<string,string>, formattedAddress?:string, lat?:number, lng?:number){
...
Deno.serve(async (req)=>{
  ...
  try{
    if(body.op==='autocomplete'){
      ...
      if(!r.ok) r = await autoLegacy(key, input); // fall back if New Places API isn't enabled
