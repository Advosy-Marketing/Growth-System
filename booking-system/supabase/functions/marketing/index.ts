...
const CACHE_MIN=20;
// Attribute a contact to a marketing channel from its source + tags.
// Returns null when nothing recognizable is present (-> unqualified contact, NOT a lead).
// Inbound calls/texts create bare contacts (null source, no tags) and are intentionally
// excluded until a rep qualifies them by adding a channel tag.
...
// answered = the call actually connected; everything else (no-answer/busy/failed/canceled) is a miss.
const CALL_ANSWERED = st => { const s=(st||'').toLowerCase(); return s==='completed'||s==='in-progress'||s==='answered'; };

const FB_UNIVERSE_PRESET='last_90d'; // enumerate campaigns (incl. zero-spend in range) like Meta Ads Manager
async function fetchWindsor(key, dparam, startISO, endISO){
  ...
  // Facebook/Meta: pull with EXPLICIT dates so the range matches the dashboard's date filter exactly,
  // and split CORE metrics (always valid) from the best-effort lead-action field. Meta via Windsor returns
  // NOTHING for the whole query if a single requested field is unsupported — bundling actions_lead was
  // silently zeroing out all Facebook spend/impressions/campaigns. Splitting makes core metrics resilient.
  ...
  const _fbTo=new Date(endISO); _fbTo.setUTCDate(_fbTo.getUTCDate()-1); // endISO is exclusive → inclusive end date
  ...
  // LEADS — count each contact ONCE, only if CREATED inside the window, and only if it is a
  // QUALIFIED lead (has a recognizable channel via source OR tags). Bare contacts created by an
  // inbound call/text (null source + no tags) are NOT leads — they're unqualified contacts until
  // a rep tags them. (Windsor also returns a GHL row per contact-activity-day, so we dedupe by id.)
  ...
  // Facebook campaigns — core metrics + best-effort per-campaign leads, merged over a campaign "universe"
  // (last 90d) so campaigns with zero spend in the selected range still appear as zeros, like Meta Ads Manager.
  ...
  // Facebook/Meta is "connected" in Windsor if any of the FB pulls returned rows. When false, Meta Ads
  // simply isn't linked in Windsor (onboard.windsor.ai?datasource=facebook) and the UI shows a connect prompt.
...
// Phone calls come from GoHighLevel (synced to ghl_calls) — GHL collapses Twilio ring-group /
// softphone legs into ONE record per real call. answered = the call connected (status completed).
...
Deno.serve(async (req)=>{
  ...
  // Marketing is read-only and visible to the whole team: any active rep with a valid PIN can view it.
  ...
  // Phone calls (GoHighLevel) — always live from ghl_calls (not part of the Windsor cache)
  const calls = await ghlCalls(startISO, endISO);

  // Thumbtack (webhook = source of truth for Thumbtack leads + cost)
  ...
  // merge Thumbtack into the lead breakdown (replace GHL Thumbtack with webhook truth)
  ...
  // Google Ads / LSA leads — GHL rarely tags these (LSA arrives as calls), so the ad platform's own
  // reported conversions are the source of truth. Fold them into qualified leads under "Google LSA".
